...
func runDaily(session disgord.Session, msg *disgord.Message, args []string) {
	...
	if voted && topGGCalc >= 1 {
		...
		msg.Reply(context.Background(), session, &disgord.Embed{
			Color:       65535,
			Title:       "Daily",
			Description: fmt.Sprintf("Você ganhou **%d** de dinheiro, **%d** de xp\n\nStrike: **%d**", money, xp, strike),
		})
	} else {
		need := uint64(time.Now().Unix()) - galo.Daily.Voted
		if topGGCalc >= 1 && !voted {
			msg.Reply(context.Background(), session, fmt.Sprintf("%s, para pegar o bônus diario voce precisa votar em mim.\n Você pode pegar o bônus diario a cada 12 horas\nLink para votar:\nhttps://top.gg/bot/470684281102925844", msg.Author.Mention()))
		} else {
