...
func runGiveMoney(session disgord.Session, msg *disgord.Message, args []string) {
	...
	if err != nil {
		msg.Reply(context.Background(), session, msg.Author.Mention()+", Quantia inválida")
		return
	}
	if 0 >= value {
		msg.Reply(context.Background(), session, msg.Author.Mention()+", Quantia inválida")
	...
	if err == nil {
		...
		msg.Reply(context.Background(), session, fmt.Sprintf("%s, Você deu **%d** de dinheiro a %s com sucesso", msg.Author.Mention(), value, msg.Mentions[0].Username))
	} else {
		msg.Reply(context.Background(), session, msg.Author.Mention()+", Vocnao tem essa quantia use j!lootbox para ver seu dinheiro")
		return
	}
}
