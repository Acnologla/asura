...
func (adapter UserAdapterPsql) InsertManyItem(ctx context.Context, id disgord.Snowflake, items []*entities.Item, itemID int, itemType entities.ItemType, quantity int) error {
	var itemUpdate *entities.Item
	for _, item := range items {
		if item.Type == itemType && item.ItemID == itemID {
			itemUpdate = item
